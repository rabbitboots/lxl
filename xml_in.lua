-- XML in: Converts an XML string to a nested Lua table.
-- (Use this module through `xml.lua`)
...
local stringProc = require(PATH .. "xml_lib.string_proc")
local stringWalk = require(PATH .. "xml_lib.string_walk")
...
function sym.NameStartChar(W)
	...
	local code, u8_str = utf8Tools.codeFromString(W.S, W.I)
...
local function _CharRef(W)
	if W:lit("&#") then
		...
		local u8_str, err = utf8Tools.stringFromCode(code)
		W:assert(u8_str, err)
...
local function assertUTF16Conversion(str, big_endian)
	local err_i, err
	str, err_i, err = utf8Conv.utf16_utf8(str, big_endian)
	if not str then
